...
export async function GET(request: Request) {
  ...
  const job = {
    ...
    properties: {
      ...
      application_url: { type: 'string', nullable: true, description: 'Direct application form URL; triggers programmatic indexing' },
      company_summary: { type: 'string' },
      connection_note: { type: 'string' },
      form_index: { type: 'object', readOnly: true, description: 'Scraped fields (key, label, kind, required, options, open), status, minutes, note, url and indexed_at. Partial estimates exclude conditional and later steps.' },
  ...
  return Response.json({
    ...
    paths: {
      ...
      '/api/jobs/{slug}/index': { parameters: [idParam('slug', 'Job slug')], post: { summary: 'Programmatically index the application form', security: authed, responses: { '200': { description: 'Job with refreshed form_index' }, '404': { description: 'Not found' } } } },
      '/api/hunter': { get: { summary: 'Read shortlist, session, XP completion ledger and answer library', security: authed, responses: { '200': { description: 'Hunter state' } } }, post: { summary: 'Change focus session state', security: authed, requestBody: { content: { 'application/json': { schema: { type: 'object', required: ['type'], properties: { type: { type: 'string', enum: ['select','remove','start','skip','complete','end'] }, slug: { type: 'string' }, answers: { type: 'array', items: { type: 'object', required: ['question','answer'], properties: { question: { type: 'string' }, answer: { type: 'string' } } } } } } } } }, responses: { '200': { description: 'Saved state' }, '400': { description: 'Invalid transition' } } } },
